...
class ThaiTransliterator:
    def __init__(self):
        ...
        ###encoder/ decoder
        self._encoder = Encoder(len(self._char_to_ix), self._n_h, self._emb_dim).to(device)
        self._encoder.load_state_dict(loader['encoder_state_dict'])
        self._decoder = OneStepDecoder(len(self._target_char_to_ix),self._n_s, self._emb_dim).to(device)
    ...
    def romanize(self,input_text):
        ...
        decoder_input = torch.tensor([self._target_char_to_ix["<start>"]], device=device)
        decoder_hidden = (encoder_hidden[0].reshape(1,1,self._encoder.hidden_dim),encoder_hidden[1].reshape(1,1,self._encoder.hidden_dim))

        decoded_seq = [] #output

        for di in range(self._maxlength):
            decoder_output, decoder_hidden,_ = self._decoder(decoder_input, decoder_hidden, encoder_outputs)
